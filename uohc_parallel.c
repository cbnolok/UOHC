...
extern inline ullong hashcalc(const char *str, const unsigned int concat_len, const uint32_t concat_len_reg)
{
...
extern inline ullong my_power(const unsigned int base, const unsigned int exp) //meant to be used only for positive numbers!
{
	if (exp == 0)
		return 1;
    ullong res = base;
...
int main()
{
	...
	printf("\nv2.0 CPU PARALLEL (Algorithm A).");
	#if (PARALLELIZATION)
		printf("\nCompiled with OpenMP support (multi-threaded): on a decent multi-core cpu is faster than serial.\n");
	#else
		...
		for (curlen = filename_minlen; (curlen <= filename_maxlen) && !stop; ++curlen)
		{
			...
			#pragma omp parallel private(concat) if (PARALLELIZATION)
			{
                ...
                *(concat + local_curtotlen) = '\0';
                ...
                {
                    // no need to use 64 bits registers if we know that 32 bits one will suffice (for the current max seed)
